...
#define CONTROLLER_PACKET_ID 0x20
  // ID, AMOUNT OF PACKETS
  const byte packetDescriptor[2] = {
    CONTROLLER_PACKET_ID, 0x01
  };
  SerialPort.write((byte*)(&packetDescriptor), 2);
  for(int i = 0; i < 0x01; i++){
    while(SerialPort.availableForWrite() < sizeof(array_gamepads[i])) {
    }
    SerialPort.write((byte*)(&array_gamepads[i]), sizeof(array_gamepads[i]));
    /*Serial2.write(array_gamepads[i].id);
      Serial2.write(array_gamepads[i].buttons1);
      Serial2.write(array_gamepads[i].buttons2);
      Serial2WriteShort(array_gamepads[i].lx_axis);
      Serial2WriteShort(array_gamepads[i].ly_axis);
      Serial2WriteShort(array_gamepads[i].rx_axis);
      Serial2WriteShort(array_gamepads[i].ry_axis);
      Serial2.write(array_gamepads[i].secondary_buttons);
      Serial2WriteShort(array_gamepads[i].left_trigger);
      Serial2WriteShort(array_gamepads[i].right_trigger);
      for(int j = 0; j < 6; j++){
      Serial2.write(array_gamepads[i].data[j]);
      }*/
  }
}
...
void setup() {
  ...
  // Baud rate, Serial Mode, RX pin, TX pin
  SerialPort.begin(115200, SERIAL_8N1, 3, 1);
  // Setup Callback for receiving data
  SerialPort.onReceive(serialReceive);

  // Disable I2C for UART testing
  // Wire.begin(0x20);
...
void loop() {
  ...
  for (int i = 0; i < BP32_MAX_GAMEPADS; i++) {
    ...
    if (myGamepad && myGamepad->isConnected()) {
      ...
      array_gamepads[i].id = 0x00;
      array_gamepads[i].buttons1          = 0x00;
      array_gamepads[i].buttons2          = 0x00;
      array_gamepads[i].lx_axis           = 0x00;
      array_gamepads[i].ly_axis           = 0x00;
      array_gamepads[i].rx_axis           = 0x00;
      array_gamepads[i].ry_axis           = 0x00;
      array_gamepads[i].secondary_buttons = 0x00;
      array_gamepads[i].left_trigger      = 0x00;
      array_gamepads[i].right_trigger     = 0x00;
      for(int j = 0; j < 6; j++) {
        array_gamepads[i].data[j]         = 0x00;
      }
    }
  }
